...
final class CompileCommand
{
     ...
     * @return array<string, array{file: string, parameters: list<array<string, mixed>>}>
     */
    private function buildComponentManifestEntries(array $entries): array
    {
        $manifest = [];
        foreach ($entries as $fqcn => $compiledPath) {
            $parameters = [];
            if (\is_file($compiledPath) && \is_readable($compiledPath)) {
                $compiled = \file_get_contents($compiledPath);
                if (\is_string($compiled)) {
                    $parameters = $this->collectParameterMetadata($compiled, $fqcn);
                }
            }
    ...
    /**
     * @param array<string, array{file: string, parameters: list<array<string, mixed>>}> $entries
     */
    private function buildManifestSource(array $entries): string
    {
        ...
        foreach ($entries as $fqcn => $entry) {
            $lines[] = '    ' . \var_export($fqcn, true) . " => [\n";
            $lines[] = "        'file' => " . \var_export($entry['file'], true) . ",\n";
            $lines[] = "        'parameters' => " . \var_export($entry['parameters'], true) . ",\n";
            $lines[] = "    ],\n";
        }
        $lines[] = "];\n";
    ...
    /**
     * @return list<array{kind: string, service?: string, name?: string, value?: string}>
     */
    private function collectParameterMetadata(string $source, string $fqcn): array
    {
        try {
        ...
        $return = self::findTopLevelReturn($ast);
        if (!($return instanceof Node\Stmt\Return_) || null === $return->expr) {
            return [];
        }

        $target = $return->expr;
        if ($target instanceof Node\Expr\FuncCall
            && $target->name instanceof Node\Name
            && 'fc' === $target->name->getLast()
            && ($target->args[0] ?? null) instanceof Node\Arg
        ) {
            $target = $target->args[0]->value;
        }

        if (!($target instanceof Node\Expr\Closure) && !($target instanceof Node\Expr\ArrowFunction)) {
            return [];
        }
        ...
        foreach ($target->params as $index => $parameter) {
            $name = $parameter->var instanceof Node\Expr\Variable && \is_string($parameter->var->name)
                ? $parameter->var->name
                : null;

            if (0 === $index && self::isArrayType($parameter->type)) {
                $parameters[] = ['kind' => 'props', 'name' => $name ?? 'props'];

                continue;
            }

            $serviceType = self::serviceType($parameter->type);
            if ($serviceType instanceof Node\Name) {
                $parameters[] = [
                    'kind' => 'service',
                    'name' => $name ?? '',
                    'service' => $ctx->resolveClassName($serviceType),
                ];
            ...
            if ($parameter->default !== null) {
                $parameters[] = [
                    'kind' => 'default',
                    'name' => $name ?? '',
                    'value' => $printer->prettyPrintExpr($parameter->default),
                ];

                continue;
            }

            if (self::allowsNull($parameter->type)) {
                $parameters[] = ['kind' => 'null', 'name' => $name ?? ''];

                continue;
            }

            throw new \RuntimeException(\sprintf(
                '%s: cannot compile PSX component parameter $%s metadata; use first array $props, a class/interface type, a default value, or nullable parameter.',
                $fqcn,
                $name ?? 'unknown',
            ));
        }
    ...
    private static function isArrayType(Node\Identifier|Node\Name|Node\ComplexType|null $type): bool
    {
        return $type instanceof Node\Identifier && 'array' === $type->toString();
    }

    private static function allowsNull(Node\Identifier|Node\Name|Node\ComplexType|null $type): bool
    {
        ...
        if ($type instanceof Node\UnionType) {
            foreach ($type->types as $unionType) {
                if ($unionType instanceof Node\Identifier && 'null' === \strtolower($unionType->toString())) {
                    return true;
                }
            }
        }
    ...
    private static function serviceType(Node\Identifier|Node\Name|Node\ComplexType|null $type): ?Node\Name
    {
        ...
        if ($type instanceof Node\UnionType && self::allowsNull($type)) {
            $serviceTypes = [];
            foreach ($type->types as $unionType) {
                if ($unionType instanceof Node\Identifier && 'null' === \strtolower($unionType->toString())) {
                    continue;
                }
                if (!($unionType instanceof Node\Name)) {
                    return null;
                }
                $serviceTypes[] = $unionType;
            }

            return 1 === \count($serviceTypes) ? $serviceTypes[0] : null;
        }
     ...
     */
    private static function findTopLevelReturn(array $ast): ?Node\Stmt\Return_
    {
        foreach ($ast as $statement) {
            ...
            if ($statement instanceof Node\Stmt\Namespace_) {
                foreach ($statement->stmts as $nested) {
                    if ($nested instanceof Node\Stmt\Return_) {
                        return $nested;
                    }
                }
            }
    ...
    private function doCompile(array $paths, string $cacheDir, string $manifestPath, bool $check): int
    {
        ...
        try {
            $componentManifestEntries = $this->buildComponentManifestEntries($manifestEntries);
        } catch (\RuntimeException $e) {
            $this->println("\033[31mError: " . $e->getMessage() . "\033[0m");
            return 1;
        }

        $manifestSource = $this->buildManifestSource($componentManifestEntries);
